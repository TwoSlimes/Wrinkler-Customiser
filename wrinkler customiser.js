Game.registerMod("wrinklerCustomiser",{
	init:function(){
		...
		new Game.Upgrade('Wrinkler Customiser','Allows you to customise how many wrinklers you have, what type they are, and how frequently shiny wrinklers spawn.',0,[19,8],function(){
			...
			Game.Prompt('<h3>Customise Your Wrinklers</h3>'+
				...
				'<label for"shinyChance">Chance for a Shiny:</label><input type="number" id="shinyChance" style="left:3px;width:50px;height:15px;border-radius:4px;box-shadow:0px 0px 0px 1px rgba(0,0,0,0.5) inset,0px 1px 2px rgba(0,0,0,0.5) inset;" step="0.0001" max="1"><br>'+
		...
		Game.Unlock('Wrinkler Customiser');
	...
	updateWrinklerCount:function() {
		...
		let count = Number(l('wrinklerCount').value);
		let chance = Number(l('shinyChance').value);
		if (chance == '') {chance = 0.0001}
		if (l('normalSelect').checked == true) {chance = 0}
		if (l('shinySelect').checked == true) {chance = 1}
		if (l('winklerSwitch').checked == true) {Game.WINKLERS=1} else {Game.WINKLERS=0}
		if (count != '') {
